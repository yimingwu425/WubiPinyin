target("WeaselDeployer")
  set_kind("binary")
  set_filename("WubiPinyinDeployer.exe")
  add_files("./*.cpp")
  ...
  after_build(function(target)
    if is_arch("x86") then
      os.cp(path.join(target:targetdir(), "WubiPinyinDeployer.exe"), "$(projectdir)/output/Win32")
      os.cp(path.join(target:targetdir(), "WubiPinyinDeployer.pdb"), "$(projectdir)/output/Win32")
    else
      os.cp(path.join(target:targetdir(), "WubiPinyinDeployer.exe"), "$(projectdir)/output")
      os.cp(path.join(target:targetdir(), "WubiPinyinDeployer.pdb"), "$(projectdir)/output")
    end
  end)
